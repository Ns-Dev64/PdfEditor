...
def options():
    ...
    btn_metadata = ttk.Button(window, text='Read Metadata', command=read_meta,width=20)
    btn_metadata.pack(pady=5)

    btn_extract_text = ttk.Button(window, text='Extract Text', command=extract_text,width=20)
    btn_extract_text.pack(pady=5)

    btn_extract_image = ttk.Button(window, text='Extract Image', command=extract_image,width=20)
    btn_extract_image.pack(pady=5)

    btn_en_dec = ttk.Button(window, text='Encrypt and Decrypt', command=en_dec,width=20)
    btn_en_dec.pack(pady=5)

    btn_merge = ttk.Button(window, text='Merge PDFs', command=merge,width=20)
    btn_merge.pack(pady=5)

    btn_watermark = ttk.Button(window, text='Sign/Watermark PDF', command=watermark,width=20)
    btn_watermark.pack(pady=5)

    btn_exit = ttk.Button(window, text='Exit', command=sys.exit,width=20)
